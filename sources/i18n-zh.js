const exactTranslations = new Map([
    // Page metadata and global controls
    ["Bruno's", 'Bruno 的作品集'],
    ...
    // Useful fragments split by inline HTML elements
    ['My name is', '我叫'],
    [", and I'm a", '，我是一名'],
...
const dynamicTranslations = [
    [/^Unlock at\s+(\d+)$/i, '完成 $1 项成就后解锁'],
    [/^Resets in\s+(.+)$/i, '将在 $1 后重置'],
    [/^(\d+)h\s+(\d+)min\s+(\d+)s$/i, '$1小时 $2分钟 $3秒'],
    [/^(\d+)h\s+(\d+)min$/i, '$1小时 $2分钟'],
    [/^(\d+)min\s+(\d+)s$/i, '$1分钟 $2秒'],
    [/^(\d+)s$/i, '$1秒'],
    [/^Drive\s+(\d+)km\.$/i, '驾驶 $1 公里。'],
    [/^Accept\s+(\d+)\s+cookies?\.$/i, '接受 $1 块 Cookie。'],
    [/^Finish a race in less than\s+(\d+)s\.$/i, '在 $1 秒内完成比赛。'],
    [/^Reach\s+(\d+)\s+meters?\s+high\.$/i, '到达 $1 米高度。']
]
